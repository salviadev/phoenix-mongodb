...
exports.testCreateMultitenant = function(schema, multiTenant, done) {
        ...
        pmongo.schema.importCollectionFromFile(cfg.connect, connections, schema, path.join(__dirname, '../../data/data/users.json'), { truncate: true, onImported: null }, t1)
        .then(function() {
            var odataUri = podata.parseOdataUri(util.format("/odata/master/%s(1)/photo%s", schema.name, tenentId01), "POST");
            var stream = fs.createReadStream(path.join(__dirname, '../../data/photos/john.smith.jpg'), {
                encoding: 'utf8'
            });
            return pmongo.upload.uploadBinaryProperty(cfg.connect, connections, schema, odataUri,
            ...
            var odataUri = podata.parseOdataUri(util.format("/odata/master/%s(1)/photo%s", schema.name, tenentId02), "POST");
            var stream = fs.createReadStream(path.join(__dirname, '../../data/photos/john.smith.jpg'), {
                encoding: 'utf8'
            });
            return pmongo.upload.uploadBinaryProperty(cfg.connect, connections, schema, odataUri,
            ...
            var odataUri = podata.parseOdataUri(util.format("/odata/master/%s(2)/photo%s", schema.name, tenentId01), "POST");
            ...
            var odataUri = podata.parseOdataUri(util.format("/odata/master/%s(1)%s", schema.name, tenentId01), "GET");
            return pmongo.odata.execQueryId(cfg.connect, connections, schema, odataUri);
        }).then(function(user) {
            odataUri = podata.parseOdataUri(util.format("/odata/master/%s?$filter=id eq 2%s", schema.name, tenentId01_and), "GET");
            return pmongo.odata.execQuery(cfg.connect, connections, schema, odataUri);
        }).then(function(users) {
            if (users.value.length !== 1)
                Promise.reject(new Error('User with id = 2 not found.'));
            var odataUri = podata.parseOdataUri(util.format("/odata/master/%s(1)%s", schema.name, tenentId01), "DELETE");
            return pmongo.odata.execDelete(cfg.connect, connections, schema, odataUri);
        }).then(function() {
            var odataUri = podata.parseOdataUri(util.format("/odata/master/%s(1)%s", schema.name, tenentId02), "GET");
            return pmongo.odata.execQueryId(cfg.connect, connections, schema, odataUri);
        }).then(function(user) {
            odataUri = podata.parseOdataUri(util.format("/odata/master/%s?$filter=id eq 2%s", schema.name, tenentId02_and), "GET");
            return pmongo.odata.execQuery(cfg.connect, connections, schema, odataUri);
        }).then(function(users) {
            if (users.value.length !== 1)
                Promise.reject(new Error('User with id = 2 not found.'));
            var odataUri = podata.parseOdataUri(util.format("/odata/master/%s(1)%s", schema.name, tenentId02), "DELETE");
            return pmongo.odata.execDelete(cfg.connect, connections, schema, odataUri);
        }).then(function() {
            //delete on removed record
            var odataUri = podata.parseOdataUri(util.format("/odata/master/%s(1)%s", schema.name, tenentId01), "DELETE");
            ...
            var odataUri = podata.parseOdataUri(util.format("/odata/master/%s(2)%s", schema.name, tenentId01), "DELETE");
            return pmongo.odata.execDelete(cfg.connect, connections, schema, odataUri);}).then(function() {
            ...
            var odataUri = podata.parseOdataUri(util.format("/odata/master/%s(2)%s", schema.name, tenentId02), "DELETE");
            return pmongo.odata.execDelete(cfg.connect, connections, schema, odataUri);}).then(function() {
            
        }).then(function() {
            odataUri = podata.parseOdataUri(util.format("/odata/master/%s%s", schema.name, tenentId01), "GET");
            return pmongo.odata.execQuery(cfg.connect, connections, schema, odataUri);
        }).then(function(users) {
            if (users.value.length !== 0)
                Promise.reject(new Error('Users found.'));
            ...
            odataUri = podata.parseOdataUri(util.format("/odata/master/%s%s", schema.name, tenentId02), "GET");
            return pmongo.odata.execQuery(cfg.connect, connections, schema, odataUri);
        }).then(function(users) {
            if (users.value.length !== 0)
                Promise.reject(new Error('Users found.'));
