    ...
    return __awaiter(this, void 0, Promise, function* () {
        ...
        for (let i = 0, len = collections.length; i < len; i++) {
            ...
            if (except && except.indexOf(collection.name) >= 0)
                continue;
            yield _dropCollection(db, collection.name);
        }
    });
}
    ...
    return __awaiter(this, void 0, Promise, function* () {
        if (indexes && indexes.length) {
            for (let i = 0, len = indexes.length; i < len; i++) {
                ...
                if (indexDesc.text)
                    yield _createTextIndex(collection, indexDesc.fields);
                else
                    yield _createIndex(collection, indexDesc.fields, indexDesc.unique);
            }
        }
    });
}
